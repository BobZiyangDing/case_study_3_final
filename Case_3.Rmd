...
ec_interval <- as.data.frame(quantile(ec_votes, probs = c(0.025, 0.975) ))
names(ec_interval) <- c("Number of Votes")
ec_interval <- kable(ec_interval, caption="EC Vote Total (Biden)")
...
```{r answer for North Carolina, include=F, message=F, warning=F}
temp <- as.data.frame( t(senator_sims %>% {.>48} %>% colMeans()) )

longnames <- NULL
interval_nc_senate <- data.frame(quantile(as.data.frame(senator_sims)$`North Carolina`,  probs = c(0.025, 0.5, 0.975)))

names(interval_nc_senate) <- "Vote Percentage"
...
house_all_cong_dist_share_interval <- kbl(house_all_cong_dist_share_interval, booktabs=T, caption = "House All Congressional District Vote Share Interval Estimate (REP)") %>%
  add_header_above(c(" " = 1, "Official Model" = 3, "Informative Prior" = 3, "Imputation Perturbation" = 3)) %>% 
  kable_styling(latex_options = "striped") %>%
  kable_styling(latex_options = "HOLD_position")
...
Our model predicts that Biden will win with a percentage of 67.4% and the electoral college vote will be 303 (rounded from 303.4) with the 95% confidence interval being (230, 387) votes as shown in Table 1. Distribution of th EC vote distribution is shown below. Futhermore, probability of each state Biden's chance of winning is shown in Appendix D Figure 3, and his interval estimated vote share in Appendix D Figure 4.
...
Our model predicts that the Republican Party will remain in control of the Senate with only a probability of 20%, meaning it is more likely that the Democratic Party will be in control of the Senate. The distribution of total Republican Senators is shown below in the figure. The probabilities of Republican wins the senator election in each running states are shown in Appendix D Table 5, with the interval estimate of vote share in each running states shown in Appendix D Table 6.

```{r 5.2 result, echo=F, fig.align='center', fig.width=5, fig.height=2.7, fig.pos='H'}
...
In North Carolina, Tom Tillis is predicted to win the Senate seat with a probability of 47.3%, with a 95% confidence interval of his vote share percentage as (45.7, 50.7), shown in Table 2 at the top of the page.
...
The result is also attached in Table 3, 4, 5, 6, 7, 8 in  Appendix D. In all these tables, the "Official Model" column gives point or interval estimates using the prior in section 3, the model section, whereas the "Informative Prior" column gives all the same results using the above set of prior.
...
Similarly, before and after perturbation, the results are all attached in Table 3, 4, 5, 6, 7, 8 in Appendix D. This time, the "Imputation Perturbation" column contains all the estimated results. We still observe no major difference between the estimated probability or vote share interval estimates. Thus, small errors introduced in or imputation won't affect the final prediction too much. Thus, the results generated upon imputed values are credible.
